...
void UpdateBounds(Branch &branch, Branch& pseudoBranch) {
    double branchValue = branch.GetValue();
    double pseudoBranchValue = pseudoBranch.GetValue();
    double lowerBound = LB.back() + branchValue*pseudoBranchValue,
            upperBound = UB.back() - pseudoBranchValue*(1 - branchValue);
...
Branch CumulativePairConnectivity(H &H, Branch &pseudoBranch) {
    if (StopCompute) {
        return Branch::GetZero();
    }
    ...
    PairConnectivityCalls++;
    if (BridgeReduction(H)) {
        Branch zero = Branch::GetZero();
        UpdateBounds(zero, pseudoBranch);
        return zero;
    }
    if (H.GetNodes().size() < MAX_DIMENSIONAL) {
        UpdateBounds(H.GetFN().front(), pseudoBranch);
        return SimpleCase(H.GetFN(), pseudoBranch);
    }
    ...
    Branch allowingBranch = GetAllowingBranch(H);
    if (Branch::IsUnacceptableBranch(allowingBranch)) {
        throw "CumulativePairConnectivity: strange allowingBranch";
    }
    ...
    if (!HwithRemovedBranch.IsSNconnected()) {
        UnconnectedHypernetsCount++;
        Branch zero = Branch::GetZero();
        UpdateBounds(zero, pseudoBranch2);
        if (HwithReliableBranch.HasReliablePath()) {
            ReliableHypernetsCount++;
            Branch unity = Branch::GetUnity();
            UpdateBounds(unity, pseudoBranch1);
            return pseudoBranch1*Branch::GetUnity();
        } else {
            return CumulativePairConnectivity(HwithReliableBranch, pseudoBranch1);
        }
    } else {
        if (HwithReliableBranch.HasReliablePath()) {
            ReliableHypernetsCount++;
            Branch unity = Branch::GetUnity();
            UpdateBounds(unity, pseudoBranch1);
            return pseudoBranch1*unity  + CumulativePairConnectivity(HwithRemovedBranch, pseudoBranch2);
        } else {
            return CumulativePairConnectivity(HwithReliableBranch, pseudoBranch1) +
                   CumulativePairConnectivity(HwithRemovedBranch, pseudoBranch2);
        }
    }
}